---
title: "Week 4, Session 4 — Non-parametric tests"
...
format:
  html:
    ...
    code-tools: true
  revealjs:
    output-file: "lab_week4_session4_slides.html"
...
```{r}
pairwise.wilcox.test(df_c$y, df_c$centre, p.adjust.method = "holm")
```

## 5. Concluding statement

> **Paired comparison (A).** 25 patients, median change =
> `r round(median(df_a$delta), 2)`; Wilcoxon signed-rank *p* =
> `r signif(wt_a$p.value, 2)`, 95% CI on the Hodges-Lehmann estimate
> `r round(wt_a$conf.int[1], 2)` to
> `r round(wt_a$conf.int[2], 2)`. Sign test *p* =
> `r signif(sign_test$p.value, 2)`.
>
> **Two-group (B).** Mann-Whitney *p* =
> `r signif(wt_b$p.value, 2)`, 95% CI
> `r round(wt_b$conf.int[1], 2)` to
> `r round(wt_b$conf.int[2], 2)`.
>
> **Three-group (C).** Kruskal-Wallis χ² =
> `r round(kw_c$statistic, 2)`, df = `r kw_c$parameter`,
> *p* = `r signif(kw_c$p.value, 2)`. Pairwise Wilcoxon tests with
...
::: {.notes}
Remind the audience that a *t*-test with log-transformed data is
often a cleaner answer than a Wilcoxon on the raw data.
:::
...
::: {.vgwort-pixel}
```{=html}
<!-- VG Wort Zählmarke — public ID: 0ff863ef128b475a80bdfcd525024402 -->
<img src="https://vg09.met.vgwort.de/na/0ff863ef128b475a80bdfcd525024402"
