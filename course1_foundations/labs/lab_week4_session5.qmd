...
format:
  html:
    ...
    code-tools: true
  revealjs:
    output-file: "lab_week4_session5_slides.html"
...
The formula calls for about `r ceiling(pwr_calc$n)` per arm.
...
> In a simulated two-arm trial (*n* = `r n` per arm), mean change in
> systolic blood pressure was lower in the drug arm than in placebo by
> `r round(-diff(fit$estimate), 1)` mmHg (95% CI:
> `r round(-fit$conf.int[2], 1)` to `r round(-fit$conf.int[1], 1)`;
> *p* = `r signif(fit$p.value, 2)`).
...
::: {.vgwort-pixel}
```{=html}
<!-- VG Wort Zählmarke — public ID: 0ffc5b102dd34471a88617bad962ba97 -->
<img src="https://vg09.met.vgwort.de/na/0ffc5b102dd34471a88617bad962ba97"
